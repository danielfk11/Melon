...
const QUEUES = [
  ...
  { name: 'users.login',          dlq: null,                       ttl: 30000,  msgs: (i) => ({ event: 'user_login',       userId: 5000 + i, ip: `192.168.1.${i + 10}` }) },
  ...
  { name: 'notifications.push',   dlq: null,                       ttl: 30000,  msgs: (i) => ({ deviceToken: `tok-${i}`, title: 'New offer!', body: `Discount ${10 * i}%` }) },
  ...
  { name: 'logs.application',     dlq: null,                       ttl: 15000,  msgs: (i) => ({ level: ['info','warn','error'][i % 3], service: 'api', msg: `Log entry #${i}` }) },
  { name: 'logs.audit',           dlq: 'logs.audit.dlq',           ttl: 300000, msgs: (i) => ({ action: ['create','update','delete'][i % 3], resource: 'order', actor: `admin-${i}` }) },
  { name: 'analytics.pageview',   dlq: null,                       ttl: 10000,  msgs: (i) => ({ page: `/products/${i}`, sessionId: `sess-${Date.now()}`, durationMs: 1000 + i * 200 }) },
  { name: 'analytics.click',      dlq: null,                       ttl: 10000,  msgs: (i) => ({ element: `btn-buy-${i}`, page: '/checkout', timestamp: Date.now() }) },
