...
if [[ -z "${RELEASE_TAG}" ]]; then
  ...
  echo "Use one of:"
  echo "  RELEASE_TAG=v1.2.0 NUGET_API_KEY=*** ./scripts/release-nuget.sh"
  echo "  NUGET_API_KEY=*** ./scripts/release-nuget.sh --tag v1.2.0"
  exit 1
fi
...
if [[ "${PROP_VERSION}" == v* ]]; then
  ...
  echo "Use version without 'v' prefix. Example: 1.2.0"
  exit 1
fi
